...
#include <unistd.h>//lets us use the sleep function
#include <iomanip> // for set width function
// included required library files
using namespace std;
class customer // customer class our base class
{
	//private:
	public:
	string customername;
...
class rent : public customer // inhereted class from customer class with is a r/n
//rent is customer
{
	public:
 ...
 if(carmodel=="A")
 {
 	system("CLS");
		...
		cout<<"You have choosed Tesla model 2011"<<endl;
		 ifstream inA("A.txt"); //displaying details of model A using ifstream
         char str[200];//character array which is same as a string
         while(inA) {
         inA.getline(str, 200);  
         if(inA) 
		 cout << str << endl;
...
obj1.welcm(); //welcm function is called
rent obj2; 
